...
class TimestampFormatTest {
    ...
    @Test
    fun todayShowsPrefixAndClock() {
        assertEquals("今天 10:00", formatTimestamp(now - 2 * 3_600_000, now, utc))
        // 个位小时补零
        assertEquals("今天 01:00", formatTimestamp(now - 11 * 3_600_000, now, utc))
    }

    @Test
    fun yesterdayCarriesTime() {
        assertEquals("昨天 12:00", formatTimestamp(now - 86_400_000, now, utc))
    }

    @Test
    fun dayBeforeYesterdayCarriesTime() {
        assertEquals("前天 12:00", formatTimestamp(now - 2 * 86_400_000, now, utc))
    }

    @Test
    fun earlierSameYearShowsMonthDayAndTime() {
        // 2026-03-01T12:00:00Z → 同年更早
        assertEquals("3月1日 12:00", formatTimestamp(1_772_366_400_000L, now, utc))
    }

    @Test
    fun crossYearShowsFullDateAndTime() {
        // 2025-12-31T12:00:00Z → 跨年
        assertEquals("2025年12月31日 12:00", formatTimestamp(1_767_182_400_000L, now, utc))
    }
    ...
    fun yearBoundaryDayBeforeYesterday() {
        ...
        assertEquals("前天 00:30", formatTimestamp(1_767_141_000_000L, jan2, utc))
    }
}
