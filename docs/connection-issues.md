...
### 2026-08-29 提问透传全链路实测通过（app/bridge 共同验证）
- **验证链路**（服务端日志实证）：16:22:30 `MUX 转发桌面提问 rpc=1b99abb4 → 手机` →
  手机半屏弹窗作答 → `CMD answer_question` → `RESPOND /api/respond 已接受` → 桌面端收到回答。
  custom:null 归一化修复生效，无 bad-response。
- **附带发现**：该次连接 16:22:16 建立后持续 11+ 分钟零自断——近期所有断开均为
  主动操作（断开按钮/重装 APK），旧 APK 时代的周期性自断未再出现。
