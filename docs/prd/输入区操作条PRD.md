...
| **按钮行（操作条）** | 输入框**下方**的一行横向工具条，四项从左到右固定：选择模型、查看上下文占用、终止循环、发送消息 |
...
| **上下文占用百分比** | `usedTokens / contextWindow × 100%`（`usedTokens = projectedTokens ?? pressureTokens`），向上 clamp 至 100%（与 Web `contextOccupancy()` 完全一致） |
...
- **输入框行**：输入框**从左到右撑满一整行**（`fillMaxWidth`），无 trailingIcon（上下文环在按钮行，见下）。
- **按钮行（输入框下方）从左到右固定顺序**：① 选择模型 → ② 查看上下文占用（环形进度入口） → ③ 终止循环（**正圆形**） → ④ 发送消息（最右，**正圆形**）。
- 输入框样式（`OutlinedTextField` 圆角 22dp、蓝边、`maxLines=4`、placeholder「发指令给DeepSeek Harness」）**保持不变**；技能入口上移到入口列表、上下文环保留在按钮行第 2 项。
...
- **点击弹窗 —— 与桌面端 DSH Web 完全一致**（已只读调研 `dsh-client-ui-conversation` 的 `ContextMeter` 实现，见附录 A）。桌面端弹层字段 → 手机完整映射表：

  | 桌面端字段 | 数据来源（wire `ContextUsageWire`） | 手机展示 |
  |---|---|---|
  | 标题「上下文已用」 | locale 固定文案 | 标题「上下文已用」 |
  | 百分比 `{percent}%` | `percent`（服务端算好） | 大字号百分比 |
  | 已用/总量 `~{usedTokens} / {contextWindow}` | `usedTokens = projectedTokens ?? pressureTokens`、`contextWindow` | 「~X / Y」紧凑 token 格式（`formatTokens`：517 / 12.2K / 517K / 1.2M） |
  | 三色分段条形图 | `breakdown`{systemTokens/toolsTokens/messageTokens} 按占比分段 | 三色分段条（蓝灰 / 紫 / 蓝） |
  | 图例「系统提示词」 | `breakdown.systemTokens` | 色块 + 标签 + `~token` 数 |
  | 图例「工具」 | `breakdown.toolsTokens` | 色块 + 标签 + `~token` 数 |
  | 图例「对话消息」 | `breakdown.messageTokens` | 色块 + 标签 + `~token` 数 |

- **分类口径说明（重要）**：DSH Web 把 **user 与 assistant 合并进「对话消息」**，tool 结果也属于对话 surface；「工具」专指 tool schema。三类均用 meter 的**固定启发式**（4 字符/token + 结构开销），**三者之和 ≠ 占用分子**（`projectedTokens` 是 provider 锚定的），弹层标题标注「近似组成」而非「总计」（对齐 token-meter README「present as approximate composition, never as a total」）。
- **breakdown 缺失降级**：`breakdown == null` 时只显示「标题 + 百分比 + 已用/总量」，不显示分段条与图例（对齐桌面端 breakdown 缺失时 segments 回退单色、rows 不渲染）。
- **百分比是近似参考值**：切换模型会把新容量与上一路由的采样短暂配对，直到下一请求上报 usage。明确标注「参考值，非计费/门控输入」。

### 2.4 终止循环按钮

- **形态**：**正圆形**按钮（`CircleShape` + 固定直径如 48dp + `contentPadding=0`，直径与按钮行高协调），位于按钮行第 3 项。
- **始终存在**（不再条件渲染）。
...
| # | 改动 | 现状 → 目标 | 备注 |
|---|---|---|---|
| 1 | **输入区三行布局（重构）** | `Row(输入框+按钮)` 单行 → `Column(入口列表行 + 输入框撑满 + 按钮行四项)` | `ConversationComposer` |
| 2 | **模型入口 + 选择弹窗** | 无 → 按钮行第 1 项，弹窗选 provider/模型/effort，`set_model` 命令下发 | 数据源 = bridge `models`（附录 C） |
| 3 | **上下文环形进度 + 明细弹窗** | 无 → 按钮行第 2 项环形入口，点击弹**完整字段明细**（百分比/已用总量/三色分段条/三行图例，与桌面端一致） | 数据源 = bridge `contextUsage` |
| 4 | **技能面板（新增）** | 无 → 输入框上方入口列表「技能」chip，弹半屏面板浏览全部技能（name+description+whenToUse）+ 可搜索；入口列表数据驱动可扩展 | 数据源 = bridge `skills_update`（全局） |
| 5 | **终止循环按钮常驻两态（正圆形）** | 仅 running 渲染红色圆钮 → 按钮行第 3 项常驻**正圆形**；running 红可点 / 非 running 灰不可点 | `enabled = agentRunning` |
| 6 | **发送按钮两态（正圆形）** | 始终可点（空输入 no-op）→ 按钮行第 4 项（最右）**正圆形**；非空可点、空/发送后置灰 | 置灰条件见 2.5 解 A |
