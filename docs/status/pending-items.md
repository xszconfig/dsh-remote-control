...
2. **Kotlin LSP 二进制更新**——✅ 已实施：稳定版不存在（项目 pre-alpha），按决策退回新 EAP 263.4421.0（arm64 原生，过期自毙已修复，服务器存活）；⚠️ LSP-1561 Android 导入修复上游尚未发布二进制（javap 反汇编已证实旧 cast 仍在，Kotlin 诊断暂为假阴性空结果）——待 JetBrains 发布含 4be8d16815ea 的构建后按 UPGRADE-NOTES 再升级
