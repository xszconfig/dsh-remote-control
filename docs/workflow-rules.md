...
- **简单检查用一条命令收尾，禁止过度 thinking**：状态类验证（重启成功没、服务起来没）就是一条 curl/一条命令的事，查完直接报结果，不做多余推理和多轮排查；
- **编译通过 → 自动安装到华为真机 → 自动验收**：不用等用户指示；安装一律用 `huawei-adb-fast-install` skill（失败再回退 `huawei-adb-install`）；装完自动做验收（启动 App、确认连接成功、核心页面正常、无崩溃），验收结果直接汇报。
